...
public class UpperStackManager : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        Sync.Receive("removeFromArItemView", (string receivedUrl) =>
        {
            ...
            for (var i = arListViewItems.Count - 1; i >= 0; i--)
            {
                ...
                // Remove qutation marks from received url and compare it with the url of the item
        ...
        if (arListViewItems.Count > 0)
        {
            ...
            if (arListViewItems.Count > 0) // && IsLeadingItem(arListViewItems[^1]))
            {
                arListViewItems[^1].SetStackedPosition(new Vector2(
                    ...
                    Mathf.Min(arListViewItems[^1].rectTransform.rect.height, arListViewItems[^1].logicalPosition)), true);
                ...
                for (int i = arListViewItems.Count - 2; i >= 0; i--)
                {
                    arListViewItems[i].SetStackedPosition(new Vector2(
                        ...
                        arListViewItems[^1].GetStackedPosition().y + (arListViewItems[^1].rectTransform.rect.height * (arListViewItems.Count - i - 1))), true);
                }
            }
        }
    }
    ...
    {
        return aRListViewItem.logicalPosition <= 0;
    }

    private bool IsLeadingItem(ARListViewItem aRListViewItem)
    {
        // Debug.Log(aRListViewItem.logicalPosition);
        // Debug.Log(lowerLeadingBounds);
        // Debug.Log((lowerLeadingBounds + aRListViewItem.rectTransform.rect.height));
